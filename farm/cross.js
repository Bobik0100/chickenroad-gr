...
function say(text,mood){calloutEl.textContent=text;calloutEl.classList.remove("coop-callout-glad","coop-callout-sad");if(mood==="glad"){calloutEl.classList.add("coop-callout-glad-8c1d6952");}else if(mood==="sad"){calloutEl.classList.add("coop-callout-sad-8c1d6952");}}
function showBanner(text,won){bannerEl.textContent=text;bannerEl.className="coop-banner-8c1d6952"+
(won?"coop-banner-win":"coop-banner-lose");bannerEl.hidden=false;}
function hideBanner(){bannerEl.hidden=true;bannerEl.textContent="";}
function buildRoad(){stripEl.innerHTML="";dotsEl.innerHTML="";laneTiles=[];dotEls=[];curbEl=document.createElement("div");curbEl.className="coop-lane-8c1d6952 coop-curb-8c1d6952";var curbTag=document.createElement("span");curbTag.className="coop-lane-mult-8c1d6952";curbTag.textContent="Start";curbEl.appendChild(curbTag);stripEl.appendChild(curbEl);for(var i=0;i<LANE_TOTAL;i+=1){var tile=document.createElement("div");tile.className="coop-lane-8c1d6952";var tag=document.createElement("span");tag.className="coop-lane-mult-8c1d6952";tag.textContent=LADDER[i].toFixed(2)+"×";tile.appendChild(tag);stripEl.appendChild(tile);laneTiles.push(tile);var dot=document.createElement("li");dot.className="coop-dot-8c1d6952";dotsEl.appendChild(dot);dotEls.push(dot);}
henEl=document.createElement("span");henEl.className="coop-hen-8c1d6952";henEl.setAttribute("aria-hidden","true");henEl.textContent="🐔";curbEl.appendChild(henEl);}
function clearRoad(){for(var i=0;i<LANE_TOTAL;i+=1){var tile=laneTiles[i];tile.classList.remove("coop-lane-clear","coop-lane-hit");var extras=tile.querySelectorAll(".coop-car-8c1d6952, .coop-feather-8c1d6952");for(var k=0;k<extras.length;k+=1){tile.removeChild(extras[k]);}
dotEls[i].classList.remove("coop-dot-done-8c1d6952");}
